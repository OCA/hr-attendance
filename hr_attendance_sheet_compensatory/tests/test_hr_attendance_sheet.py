...
class TestHrAttendanceSheet(TransactionCase):
    def setup_employee(self):
        ...
        self.user_admin = self.env["res.users"].browse(1)
        self.employee_admin = self.env["hr.employee"].create(
            {
                "name": "Employee Admin",
                "tz": "UTC",
    ...
    def setUp(self):
        ...
        self.HrAttendanceSheet = self.env["hr.attendance.sheet"]
        ...
        self.attendance_config = self.AttendanceSheetConfig.create(
            {
    ...
    def test_name_get_missing_employee(self):
        with freeze_time("2021-12-12 20:45", tz_offset=0):
            new_element = self.HrAttendanceSheet.new({})
            self.assertEqual(new_element.name_get()[0][1], "False")

    def test_require_regeneration(self):
        validation_sheet = self.HrAttendanceSheet.create(
            {
                "employee_id": self.employee.id,
    ...
    def test_name_get_multi(self):
        ...
        weeks += self.HrAttendanceSheet.create(
            {
                "employee_id": self.employee.id,
                "start_date": "2021-12-13",
                "end_date": "2021-12-19",
        ...
        self.assertEqual(res[0][1], "Employee 1 (12/06/21 - 12/12/21)")
        self.assertEqual(res[1][1], "Employee 1 (12/13/21 - 12/19/21)")

    def test_default_from_date(self):
        with freeze_time("2021-12-12 20:45", tz_offset=0):
            new_element = self.HrAttendanceSheet.new({})
            self.assertEqual(new_element.start_date, date(2021, 11, 29))
            self.assertEqual(new_element.end_date, date(2021, 12, 5))

        with freeze_time("2021-12-13 06:45", tz_offset=0):
            new_element = self.HrAttendanceSheet.new({})
            self.assertEqual(new_element.start_date, date(2021, 12, 6))
            self.assertEqual(new_element.end_date, date(2021, 12, 12))
    ...
    def test_action_retrieve_attendance_and_leaves(self):
        validation = self.HrAttendanceSheet.new(
            {
                "employee_id": self.employee.id,
            }
        )
        ...
        validation.start_date = "2021-12-06"
        validation.end_date = "2021-12-12"
        validation.action_retrieve_attendance_and_leaves()
    ...
    def test_action_retrieve_leaves_outer_validation_date(self):
        ...
        validation.start_date = "2021-12-07"
        validation.end_date = "2021-12-08"
        validation.action_retrieve_attendance_and_leaves()
    ...
    def test_generate_compensatory(self):
        ...
        self.assertEqual(validation.state, "done")
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            "validation reviews: Employee 1 (12/06/21 - 12/12/21)",
        )
    ...
    def test_generate_leaves(self):
        ...
        validation = self.HrAttendanceSheet.create(
            {
                "employee_id": self.employee2.id,
                "start_date": "2021-12-06",
                "end_date": "2021-12-12",
        ...
        validation.action_retrieve_attendance_and_leaves()
        validation.action_attendance_sheet_confirm()
        validation.action_attendance_sheet_done()
        self.assertEqual(validation.state, "done")
        ...
        self.assertEqual(
            ...
            "Employee 2 without user (12/06/21 - 12/12/21)",
        )
    ...
    def test_generate_reviews(self):
        reviews = self.HrAttendanceSheet._create_sheet_id()
        self.assertEqual(
            len(reviews),
    ...
    def test_avoid_duplicated_allocation(self):
        ...
        attenance_review_week.action_attendance_sheet_confirm()
        attenance_review_week.action_attendance_sheet_done()
        self.assertEqual(
            self.env["hr.leave.allocation"].search_count([]), count_before + 1
        )
        self.assertEqual(attenance_review_week.state, "done")
        self.assertTrue(attenance_review_week.leave_allocation_id)
    ...
    def test_user_can_read_validated_sheet_only(self):
        ...
        HrAttendanceSheetEmployee = self.HrAttendanceSheet.with_user(self.user_employee)
        self.assertEqual(HrAttendanceSheetEmployee.search_count([]), 0)
        with self.assertRaisesRegex(AccessError, "Due to security restrictions.*"):
            validation.with_user(self.user_employee).read(["start_date"])
        validation.action_attendance_sheet_confirm()
        validation.action_attendance_sheet_done()
        self.assertEqual(HrAttendanceSheetEmployee.search_count([]), 1)
        validation.with_user(self.user_employee).read(["start_date"])
